  ...
  return (
    <div className="space-y-2">
      {seasons.map(season => {
        ...
        return (
          <div key={season.id} className="rounded-2xl overflow-hidden backdrop-blur-md" style={glassCard}>
            <button
                ...
                <div className="flex items-center gap-2">
                  <div className="w-24 h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.1)' }}>
                    <div
                      ...
                      style={{ width: `${(watchedCount / season.episode_count) * 100}%`, background: '#f97316' }}
                    />
                  </div>
                ...
                {Array.from({ length: season.episode_count }, (_, i) => i + 1).map(ep => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        background: watched ? 'rgba(249,115,22,0.15)' : 'rgba(255,255,255,0.04)',
                        border: watched ? '1px solid rgba(249,115,22,0.3)' : '1px solid rgba(255,255,255,0.07)',
                      }}
                    >
                      <span className="text-sm font-medium" style={{ color: watched ? '#f97316' : '#a1a1aa' }}>
                        E{ep}
                      </span>
                    </button>
                  )
